...
> 文档版本：2.2（单房间私用流程修订）
> 基准日期：2026-07-15
> 本地仓库：`C:\Users\fj\Documents\realtime-vedio-sharing`  
> 本地开发机：Windows x64、PowerShell 7.5、Conda 25.1.1（`win-64`）；项目工具链由仓库内隔离环境提供  
...
### 2026-07-15 已实施修订（优先于本文旧版对应条款）

本节是已落地的产品与部署决策；本文后续仍出现的“房间编号、房间密码、管理员用户名/复杂密码、仅备案域名正式可用”等旧版描述均由本节覆盖：

- 系统只允许一个活动房间。好友使用长期固定且不可猜测的邀请路径，打开后只输入昵称，不输入房间编号或口令。
- 控制后台不输入账号，仅输入固定六位数字控制口令 `260713`；口令不定时轮换。服务端仍只保存 Argon2id 哈希，不以明文写入数据库。
- 开启放映控制时只输入主持人昵称；后台持续显示活动房间人数、节目模式、当前影片或 OBS 发布状态，并提供强制关闭房间、撤销会话和清退 RTC 连接的控制。
...
上述实现由 `migrations/005_single_room_console.sql`、应用契约、前端页面、服务器 Compose、`tools/server/deploy-ip-test.sh` 和 `tools/server/ip-cert.sh` 共同约束，并由集成、安全、媒体、Worker、浏览器及线上协议测试验收。
...
- 一个管理员账户；朋友通过房间链接、昵称和共享房间密码加入，不开放注册。
...
实施时创建以下有效 YAML；随后在本机 Windows x64 上生成 Conda explicit spec `conda-win-64.lock` 并提交：
...
```powershell
Set-Location -LiteralPath 'C:\Users\fj\Documents\realtime-vedio-sharing'
$env:CONDA_PKGS_DIRS = Join-Path $PWD '.cache\conda-pkgs'
$env:TEMP = Join-Path $PWD 'tmp'
$env:TMP = $env:TEMP
New-Item -ItemType Directory -Force -Path $env:CONDA_PKGS_DIRS, $env:TEMP | Out-Null
conda env create --prefix (Join-Path $PWD '.conda\envs\dev') --file environment.yml
conda list --prefix (Join-Path $PWD '.conda\envs\dev') --explicit | Set-Content -Encoding utf8 conda-win-64.lock
```
...
$env:TMP = $env:TEMP
New-Item -ItemType Directory -Force -Path $env:CONDA_PKGS_DIRS, $env:TEMP | Out-Null
conda create --yes --prefix (Join-Path $PWD '.conda\envs\dev') --file conda-win-64.lock
```

约束：

- 不执行 `conda init`，不更新或修改 base 环境。
- 不调用本机全局 Node 22.18.0 和 pnpm 11.7.0；所有项目命令走项目包装器。
- 根 `package.json` 固定精确的 `packageManager`，`pnpm-lock.yaml` 必须提交，CI 使用 `--frozen-lockfile`。
- `tools/environment/run-dev.ps1` 统一设置 `TEMP`、`TMP`、`XDG_CACHE_HOME`、`CONDA_PKGS_DIRS`、`PNPM_HOME`、pnpm store 和 `PLAYWRIGHT_BROWSERS_PATH`，再使用绝对 Conda prefix 执行命令。
- 文档中的裸 `pnpm` 在本地 Windows 开发环境中表示 `pwsh -File tools/environment/run-dev.ps1 pnpm`，实施脚本不得依赖当前 PowerShell 是否激活 Conda。
...
- `max_members=5`，管理员也计数。
- 管理员创建/关闭唯一活动房间，房间 ID 使用 128 位随机值。创建请求同时提供 `hostNickname`，事务内创建管理员对应的room member/session并设为host；因此管理员从创建时起就计入5人上限。
- 共享密码使用 Argon2id；轮换密码只阻止新加入，不自动踢出当前会话。管理员可选择“轮换并撤销全部成员会话”。
- 加入成功签发 12 小时 HttpOnly session Cookie；昵称和密码失败分别限速。
- 主持人可转交；被转交者必须在线。
- 主持人断线保留 30 秒 lease；重连恢复。超时后管理员在线则回到管理员，否则转给在线时间最长的成员。
- 显式leave立即撤销session、设置 `left_at`、释放5人名额；若离开者是host则不等待30秒，立即按相同优先级移交。以后用相同归一化昵称加入会创建新的member ID，旧记录只作审计，不复用旧session。
...
- 管理 Cookie：`sw_admin`；房间 Cookie：`sw_room`；均为 `Secure; HttpOnly; SameSite=Strict; Path=/`，host-only，不设置 Domain。
...
| 方法与路径 | 权限 | 请求 | 成功响应 |
|---|---|---|---|
| `POST /admin/login` | 公网限速 | username/password | admin session、CSRF、admin |
| `POST /admin/logout` | admin | admin CSRF | 204并仅撤销admin session；room session由leave/close处理 |
| `POST /rooms` | admin | password、hostNickname、maxMembers=5 | room summary、join URL，并为管理员建立host member/session |
| `PATCH /rooms/:id` | admin | rotatePassword/close | room summary |
| `POST /rooms/:id/join` | 公网限速 | nickname/password | room session、CSRF、member |
...
- `/`：房间链接/ID、昵称、共享密码、设备兼容提示。
- `/room/:roomId`：节目画面、字幕、成员、麦克风、播放控制和音量。
- `/admin`：登录、房间、媒体、上传、字幕、OBS配置、成员、安全操作。
...
1. 运维在服务器TTY执行 `admin-bootstrap` CLI，浏览器打开HTTPS入口。
2. 管理员登录 `/admin`，创建唯一活动房间，设置共享密码，复制加入链接。
...
1. 打开加入链接，填写昵称和共享密码。
2. 点击“进入并启用声音”；接受麦克风权限可通话，拒绝则只听。
3. 页面先同步服务器快照，再加载当前VOD或WHEP；成员不能控制播放，除非获得主持权。
4. 成员可独立调整节目、通话总量、逐人音量，选择字幕/全屏/横屏；这些偏好不影响其他人。
5. 网络切换时页面显示“正在重连”，重新取短期凭据和快照；恢复失败时诊断页给出UDP/TCP/TURN所处阶段和脱敏复制报告。

主持操作：

- VOD：选择影片→全员预加载→服务端在统一 `effectiveAt` 播放；暂停/跳转/倍速同理。
- 直播：OBS先online再切换；直播没有共享seek，播放控制只包含切入/切出直播。
- 转交主持、踢人、轮换密码和关闭房间均需要二次确认；关闭房间立即撤销会话和RTC刷新。
...
- 管理员和房间密码 Argon2id；参数在服务器基准测试后锁定，单次验证约 250 ms且不超过内存预算。
- 登录失败：5次/分钟/IP，20次/小时/IP；房间密码同级别并叠加 room维度。
- Token接口20次/分钟/session；WS控制20条/秒/host，质量上报每5秒最多1条。
- Session固定攻击防护：登录/加入后总是轮换 ID；退出、踢人、房间关闭、密码强制轮换撤销。
